 ...
 */
async function validateEventAuthorization(eventIds, userId, userRole) {
  ...
  const unauthorizedEvent = events.find(
    (event) => event.organizer_id.toString() !== userId && userRole !== "admin"
  );
 ...
 */
async function createDiscount(discountData, userId, userRole) {
  const {
    applicable_events,
    code,
    description,
  ...
  await validateEventAuthorization(applicable_events, userId, userRole);
...
function validateDiscountData(discountData) {
  const { discount_type, discount_value, start_date, end_date } = discountData;
 ...
 */
async function getDiscountsByEvent(eventId, userId, userRole) {
  // Validate eventId
  ...
  await validateEventAuthorization([eventId], userId, userRole);
 ...
 */
async function updateDiscount(discountId, updates, userId, userRole) {
  ...
  // Validate authorization
  await validateEventAuthorization(
  ...
  if (updates.discount_type || updates.discount_value) {
    validateDiscountData({ ...discount.toObject(), ...updates });
  }
 ...
 */
async function deleteDiscount(discountId, userId, userRole) {
  const discount = await DiscountRepository.findById(discountId);
  if (!discount) {
    throw new Error("Discount code not found");
  }
 ...
 */
async function validateDiscountForEvent(
  eventId,
  code,
  ticketCount,
  ticketType
...
async function validateDiscountCode(code, subtotal) {
  ...
  const discountAmount = calculateDiscountAmount(discount, subtotal, 1);
 ...
 */
function calculateDiscountAmount(discount, price, quantity = 1) {
 ...
 * @returns {Object} - Updated discount
 */
async function applyDiscountCode(discountId) {
 ...
 */
async function getDiscountStats(discountId, userId, userRole) {
  const discount = await DiscountRepository.findById(discountId);
  if (!discount) {
    throw new Error("Discount code not found");
  }
